...
class Container(UserOwnedObject):
  ...
  def inspect(self):
    ...
    """
    self.user.dockerDaemon.getConnection().request("GET","/v1.24/containers/"+self.id+"/json")
  ...
  def stop(self):
    self.user.dockerDaemon.getConnection().request("POST","/v1.24/containers/"+self.id+"/stop")
    response = self.user.dockerDaemon.getConnection().getresponse()
    response.read()
  ...
  def remove(self,force=False):
    ...
    self.user.dockerDaemon.getConnection().request("DELETE","/v1.24/containers/"+self.id+"?"+queryParametersString)
    response = self.user.dockerDaemon.getConnection().getresponse()
    response.read()
